      ...
      {data.allFile.edges.map(edge => {
        ...
        return (
          <div className="relative max-w-xs sm:max-w-sm md:max-w-none filter group flex mx-auto">
            ...
            />
            <p className="absolute text-white hidden text-center group-hover:flex filter inset-x-2/4 inset-y-2/4 transform -translate-y-1/2 -translate-x-1/2 brightness-200 ">
              {edge.node.name}
            </p>
          </div>
        )
